...
exports.drawCrosshair = (frame) => {
	...
	const frameCenterPoint = new cv.Point( frameX, frameY );
...
exports.drawTarget = (binaryImg, frame, minPxSize, crosshair) => {
	...
	for (let label = 1; label < centroids.rows; label += 1)
	{
		...
		if (minPxSize < size)
		{
			const dist = Math.round(calcDistance(
	...
	if( targetList.length > 0 )
	{
		...
		const 	targetCenter = (t.x2 - t.x1) / 2,
				targetXPos = t.x1 + targetCenter,
				targetYPos = t.y1 + targetCenter;

		t.xPos = Math.round( (frameWidth / 2) - targetXPos );
		t.yPos = Math.round( (frameHeight / 2) - targetYPos );
